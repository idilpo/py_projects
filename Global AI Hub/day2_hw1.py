...
# Comment on the exercise: here the instruction "number" are considered as "digits" to avoid infinite loops.
odd_digits = [1, 3, 5, 7, 9]
even_digits = [0, 2, 4, 6, 8]
merged_list = odd_digits + even_digits #two lists are merged together using the built-in function "merge"
merged_list.sort() #the merged list is sorted in an incrasing order with the buil-in function "sort"
for i in merged_list: #looping thorugh the elements in the list, multiplying each by 2
    merged_list[i] *= 2
